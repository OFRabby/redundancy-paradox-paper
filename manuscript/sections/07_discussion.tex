...
\subsection{Methodological recommendations for practice}

Our work contributes not only diagnostic findings but also a \textbf{practical methodology} for building more reliable NIDS ensembles. The key insight is that LOO should be used as a \emph{diagnostic signal}---informing the practitioner about potential detector issues---rather than as an \emph{automatic selection rule}. When LOO's sign-conflict rate is low ($<$15\%), its recommendations are reliable. When sign-conflict is high, epsilon-VRG provides a principled guardrail that preserves ensemble performance while avoiding over-pruning.

This methodology generalizes beyond NIDS. Any domain using rank-averaged ensembles of heterogeneous anomaly detectors---fraud detection, medical diagnosis, manufacturing quality control---can apply the same sign-conflict diagnostic and epsilon-VRG guardrail. The decision protocol (Section~\ref{sec:guidelines}) is intentionally simple: compute sign-conflict, check threshold, apply appropriate selection method. No specialized expertise is required beyond basic ensemble construction.
